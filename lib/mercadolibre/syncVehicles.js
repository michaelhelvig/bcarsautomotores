...
async function maybeAutoPublishToInstagram(savedVehicle, { wasNew }) {
    ...
    // "Reservamos" la publicación ANTES de llamar a Instagram, con un
    // UPDATE condicionado a que instagram_media_id siga siendo NULL.
    //
    // Por qué hace falta esto: Mercado Libre reintenta el webhook de forma
    // agresiva y puede mandar la misma notificación más de una vez casi en
    // simultáneo. Antes, dos llamadas a syncSingleVehicle para el mismo
    // item podían correr en paralelo, y las DOS leían instagram_media_id
    // en null (ninguna había terminado de escribir todavía) → las dos
    // publicaban en Instagram → post duplicado. Con este UPDATE atómico,
    // solo una de las dos consigue "reservar" la fila (Postgres resuelve
    // el WHERE de forma atómica); la otra ve 0 filas afectadas y no
    // publica nada.
    const { data: claimed, error: claimError } = await supabaseAdmin
        .from('vehiculos')
        .update({ instagram_media_id: 'publishing' })
    ...
    if (claimError) {
        console.error(
            `[Instagram] No se pudo reservar la publicación (${savedVehicle.meli_item_id}):`,
            claimError,
        )
        return
    }
    ...
    if (!claimed || claimed.length === 0) {
        // Otra ejecución (webhook duplicado, sync manual en paralelo, etc.)
        // ya se adelantó a publicar este vehículo. No hacemos nada más.
        console.log(
            `[Instagram] Vehículo (${savedVehicle.meli_item_id}) ya estaba reservado/publicado, se omite.`,
        )
        return
    }
    ...
    try {
        ...
        // Se libera la reserva (vuelve a NULL) para que un próximo sync
        // pueda reintentar publicar este vehículo, y se guarda el error
        // en la fila para poder verlo después desde Supabase sin tener
        // que ir a buscar en los logs de Vercel.
        await supabaseAdmin
            .from('vehiculos')
            .update({ instagram_media_id: null, instagram_publish_error: message })
            .eq('id', savedVehicle.id)
    }
}
