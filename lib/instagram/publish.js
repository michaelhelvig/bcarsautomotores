...
//
// NOTA: no se usa en el caption actual (28/08), que quedó en texto plano
// a pedido. Se deja la función por si en algún momento se quiere volver
// a poner alguna palabra en negrita ahí.
...
// edita solo acá.
const WHATSAPP_NUMBER = '11 3662-0534'
...
function buildCaption(vehicle) {
    const { marca, modelo, anio, km, tipo } = vehicle
    ...
    const lineas = [
        ...
        `Año: ${anio}`,
        `Km: ${kmLine}`,
        '📍Para consultas o mas información no dudes en contactarnos al privado o a nuestro celular:',
        `📱 ${WHATSAPP_NUMBER}`,
        '',
        hashtags,
    ]
...
export async function publishVehicleToInstagram(vehicle) {
    ...
    // Portada con diseño (foto + degradé + marca/modelo/specs): SUSPENDIDA
    // por pedido (28/08) — por ahora se publica solo con las fotos reales
    // del vehículo, en buena calidad y resolución, sin la imagen armada
    // por código. Para reactivarla, poner COVER_ENABLED en true de nuevo.
    const COVER_ENABLED = false

    // Si por lo que sea no se puede generar o subir la portada (falla al
    // traer una fuente, etc.), se publica igual con las fotos reales del
    // auto en vez de cortar toda la publicación — mismo criterio que el
    // resto de este flujo.
    //
