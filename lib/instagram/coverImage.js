...
// Altura de la franja de foto arriba: un poco más de la mitad del canvas.
// El resto (abajo) es la zona oscura con marca/modelo/specs, centrado.
const IMAGE_HEIGHT = Math.round(HEIGHT * 0.56)

// Mismo tono oscuro que usa el resto del diseño (portada, sitio), para el
// fondo sólido de relleno de esta función.
const BACKGROUND_COLOR = { r: 0x14, g: 0x11, b: 0x10 }

// Recorta cualquier foto al mismo canvas 1080x1350 que usa la portada,
// sin texto encima, para que en el carrusel entren todas del mismo
// tamaño exacto (así Instagram no decide por su cuenta cómo encajarlas).
//
// Antes esto se hacía con objectFit:'cover', que como las fotos reales
// son horizontales (tipo 4:3) y el canvas es vertical (4:5), recortaba
// mucho de los costados — el auto se veía "zoomeado". Después se agregó
// un fondo con la misma foto difuminada + oscurecida para rellenar el
// espacio sobrante, pero quedaba raro (28/08: se saca ese difuminado).
//
// Ahora se arma en dos capas con sharp (sin pasar por Satori, no hace
// falta para esto):
//   1) Fondo: un color sólido plano (mismo tono oscuro del diseño), en
//      vez de la foto difuminada.
//   2) Primer plano: la foto ENTERA sin recortar, centrada arriba del
//      fondo — nunca se agranda si es más chica que el canvas
//      (withoutEnlargement), para no perder nitidez; si es más grande,
//      se achica lo justo para que entre completa.
// Así el auto se ve siempre completo (en miniatura y al abrir el post),
// sin recorte y sin difuminado, en la mejor resolución que permite la
// foto original.
